...
// 전역 변수 선언 (원래 코드에 있던 것들)
static uint8_t knownPlayerIDs[L3_MAXDATASIZE];
...
void L3_handleDay()
{
    ...
    // 메시지 입력 처리 (작동하는 코드와 동일한 방식)
    if (pc.readable()) {
        char c = pc.getc();
        if (!L3_event_checkEventFlag(L3_event_dataToSend)) {
            if (c == '\n' || c == '\r') {
                originalWord[wordLen++] = '\0';  // 작동하는 코드와 동일
                L3_event_setEventFlag(L3_event_dataToSend);
            } else {
                originalWord[wordLen++] = c;
                if (wordLen >= L3_MAXDATASIZE - 1) {  // 작동하는 코드와 동일
