...
int doctorTarget = -1;
int policeTarget = -1; // 조사 대상 (확인용, 딱히 상태에 영향 없음)
...
{   
    ...
    switch (main_state)
    {
       ...
       case VOTE:
        ...
        case DOCTOR:
        {
            static bool doctorDone = false;

            if (myId == 1) {
                pc.printf("\r\n[HOST] 의사 단계입니다. 게스트 의사의 입력을 기다립니다.\n");
                change_state = 3;
                main_state = NIGHT;
                break;
            }

            if (strcmp(myRoleName, "Doctor") != 0 || idead) {
                // 의사가 아니거나 죽었으면 바로 밤으로
                change_state = 3;
                main_state = NIGHT;
                break;
            }
            ...
            if (!doctorDone) {
                ...
                doctorTarget = selectedId;
                pc.printf("✅ %d번 플레이어를 살리기로 결정했습니다.\n", doctorTarget);
                doctorDone = true;
            }
